...
#define SERVO_PWM_RES_MAX (1 << SERVO_PWM_RESOLUTION)
#define SERVO_PWM_RANGE_MIN (SERVO_PWM_RES_MAX / 20) // 50Hz wave -> 20ms period / 20 == 1ms duty
#define SERVO_PWM_RANGE_MAX (SERVO_PWM_RES_MAX / 10) // 50Hz wave -> 20ms period / 10 == 2ms duty

// Mulitplier used to calc the max delay in lower-speed move operation, where
...
class Servo {
  public:
    ...
    Servo (uint16_t minPulseWidth, uint16_t maxPulseWidth, int fullMoveDelay, uint8_t pin, uint8_t inverted);
